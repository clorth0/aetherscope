...
# and writes logs to ~/Library/Logs/hackrf-web/.
#
# Override the label with HACKRF_WEB_LABEL if you want.
#
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TEMPLATE="$REPO_DIR/deploy/com.hackrf-web.plist.template"

[[ -f "$TEMPLATE" ]] || { echo "ERROR: template not found at $TEMPLATE" >&2; exit 1; }
[[ "$(uname -s)" == "Darwin" ]] || { echo "ERROR: launchd is macOS-only." >&2; exit 1; }

LABEL="${HACKRF_WEB_LABEL:-local.hackrf-web}"
...
mkdir -p "$HOME/Library/Logs/hackrf-web" "$HOME/Library/LaunchAgents"
...
for old in "$LABEL" "com.clorth0.hackrf-web"; do
...
if curl -sf -o /dev/null -w "%{http_code}\n" http://127.0.0.1:8765/ | grep -q 200; then
    ...
    echo "    tail -f ~/Library/Logs/hackrf-web/stderr.log"
fi
